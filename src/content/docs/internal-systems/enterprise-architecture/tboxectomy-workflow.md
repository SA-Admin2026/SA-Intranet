...
- On the instances of bp:Navigation, remove the gist:label triples. (Already has rdfs:label triples) AE 8/12/15 – Actually remove the rdfs:label as well. Some are typed as string and others aren't so it messes with the delete duplicates when we reimport these instances via the TBox.

DELETE { ?s gist:label ?label1 . ?s rdfs:label ?label2 }  
 WHERE {?s rdf:type bp:Navigation . ?s gist:label ?label1 . ?s rdfs:label ?label2 }
...
Delete duplicates (subject, predicate,object only). For some reason AG does not recognize the duplicate labels on two instances of bp:Navigation (sa:\_Application and sa:\_Configuration). I deleted two of these by hand. I think this is because in e6 we have the labels as datatype properties, whereas rdfs:label is actually an annotation property.   
 After further testing I don't think the fact that it was a dtp makes a difference, I think it is because the literals were registering as different because one was typed as an xsd:string and the other was not?
...
# Additional Cleanup tasks – Post 'ectomy

- Make the Project Task use case editable by putting all of the project task triples onto the same named graph

DELETE { ?uri rdf:type sa:ProjectTask .   
 ?uri rdfs:label ?task .  
 ?uri gist:categorizedBy ?status }   
 INSERT { GRAPH ?g3 { ?uri rdf:type sa:ProjectTask }   
 GRAPH ?g3 { ?uri rdfs:label ?task }   
 GRAPH ?g3 { ?uri gist:categorizedBy ?status } }  
 WHERE { ?uri rdf:type sa:ProjectTask .  
 ?uri rdfs:label ?task .  
 ?uri gist:directPartOf ?project .  
 ?uri gist:categorizedBy ?status .  
 OPTIONAL { GRAPH ?g1 { ?uri rdf:type sa:ProjectTask } }  
 OPTIONAL { GRAPH ?g2 { ?uri rdfs:label ?task } }  
 OPTIONAL { GRAPH ?g3 { ?uri gist:directPartOf ?project } }  
 OPTIONAL { GRAPH ?g4 { ?uri gist:categorizedBy ?status } }  
 }

- Make RESTful Query an editable use case. There were three entries that had no TNAN at all – for those I re-entered them and deleted the old ones. For the rest I ran this query that put them all on the same TNAN as the rdf:type statement:

DELETE { ?uri rdf:type bp:RESTfulQuery .  
 ?uri rdfs:label ?label .  
 ?uri gist:containedText ?body }   
 INSERT { GRAPH ?g1 { ?uri rdf:type bp:RESTfulQuery }   
 GRAPH ?g1 { ?uri rdfs:label ?label }   
 GRAPH ?g1 { ?uri gist:containedText ?body } }  
 WHERE { ?uri rdf:type bp:RESTfulQuery .  
 ?uri rdfs:label ?label .  
 ?uri gist:containedText ?body .   
 OPTIONAL { GRAPH ?g1 { ?uri rdf:type bp:RESTfulQuery } }  
 OPTIONAL { GRAPH ?g2 { ?uri rdfs:label ?label } }  
 OPTIONAL { GRAPH ?g3 { ?uri gist:containedText ?body } }  
 }

- The State and Country use cases are not editable because most of those were created before we had TNAN. I did go back and make sure ones missing an acronym were completed.
- Same with Client, only 7 clients were added since we implemented TNAN so only those 7 are editable
- Make Billing Rate editable – these were all created via two different use cases but now there is just one. Merge onto the same TNAN.

DELETE { ?uri gist:decimalValue ?amount .   
 ?uri gist:hasUoM ?unit . }  
 INSERT { GRAPH ?g1 { ?uri gist:decimalValue ?amount }   
 GRAPH ?g1 { ?uri gist:hasUoM ?unit } }   
 WHERE { ?uri rdf:type sa:BillingRate .   
 ?uri rdfs:label ?label .   
 ?uri gist:decimalValue ?amount .   
 ?uri gist:hasUoM ?unit .   
 ?uri gist:categorizedBy ?cat   
 OPTIONAL { GRAPH ?g1 { ?uri rdf:type sa:BillingRate } }  
 OPTIONAL { GRAPH ?g2 { ?uri rdfs:label ?label } }  
 OPTIONAL { GRAPH ?g3 { ?uri gist:decimalValue ?amount } }   
 OPTIONAL { GRAPH ?g4 { ?uri gist:hasUoM ?unit } }   
 OPTIONAL { GRAPH ?g5 { ?uri gist:categorizedBy ?cat }}  
 }

- Make the SAEmployee use case editable:

DELETE { ?uri rdf:type sa:SAEmployee .   
 ?uri gist:name ?name .   
 ?uri gist:categorizedBy ?st }  
 INSERT { GRAPH ?g4 { ?uri rdf:type sa:SAEmployee }   
 GRAPH ?g4 { ?uri gist:name ?name }   
 GRAPH ?g4 { ?uri gist:categorizedBy ?st } }  
 WHERE { ?uri rdf:type sa:SAEmployee .   
 ?uri gist:name ?name .   
  ?uri gist:categorizedBy ?st .   
 ?uri gist:hasA ?br .   
 OPTIONAL { GRAPH ?g1 { ?uri rdf:type sa:SAEmployee }}  
 OPTIONAL { GRAPH ?g2 { ?uri gist:name ?name }}  
 OPTIONAL { GRAPH ?g3 { ?uri gist:categorizedBy ?st }}  
 OPTIONAL { GRAPH ?g4 { ?uri gist:hasA ?br }}  
 }

- Make the SASubContractor use case editable:

DELETE { ?uri rdf:type sa:SASubContractor .   
 ?uri gist:name ?name .   
 ?uri gist:categorizedBy ?st }  
 INSERT { GRAPH ?g4 { ?uri rdf:type sa:SASubContractor }   
 GRAPH ?g4 { ?uri gist:name ?name }   
 GRAPH ?g4 { ?uri gist:categorizedBy ?st } }  
 WHERE { ?uri rdf:type sa:SASubContractor .   
 ?uri gist:name ?name .   
 ?uri gist:categorizedBy ?st .   
 ?uri gist:hasA ?br .   
 OPTIONAL { GRAPH ?g1 { ?uri rdf:type sa:SASubContractor }}  
 OPTIONAL { GRAPH ?g2 { ?uri gist:name ?name }}  
 OPTIONAL { GRAPH ?g3 { ?uri gist:categorizedBy ?st }}  
 OPTIONAL { GRAPH ?g4 { ?uri gist:hasA ?br }}  
 }
